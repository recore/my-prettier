...
const { addLeadingComment, addTrailingComment, addDanglingComment } = utils;

function handleOwnLineComment(comment, text, options, ast, isLastComment) {
  const { enclosingNode } = comment;
  if (handleOnlyComments(enclosingNode, ast, comment, isLastComment)) {
    return true;
  }
  return false;
}
...
function attach(comments, ast, text, options) {
  ...
  comments.forEach((comment, i) => {
    ...
    if (utils.hasNewline(text, locStart(comment), { backwards: true })) {
    ...
    } else if (utils.hasNewline(text, locEnd(comment))) {
      if (
...
function printLeadingComment(commentPath, print, options) {
  ...
  if (isBlock) {
    return concat([
      contents,
      utils.hasNewline(options.originalText, options.locEnd(comment))
        ? hardline
        : " "
    ]);
  }
...
function printTrailingComment(commentPath, print, options) {
  ...
  if (
    utils.hasNewline(options.originalText, options.locStart(comment), {
      backwards: true
    })
  ) {
    ...
    const isLineBeforeEmpty = utils.isPreviousLineEmpty(
...
function printComments(path, print, options, needsSemi) {
  ...
  path.each(commentPath => {
    ...
    if (leading) {
      ...
      if (
        utils.hasNewline(text, utils.skipNewline(text, options.locEnd(comment)))
      ) {
